...
if(!empty($wr_id)){
    $itemData = sql_fetch("SELECT * FROM g5_write_{$bo_table} WHERE wr_id = '$wr_id'");
    $metaDescraiption = strip_tags($itemData['wr_content']);
    $metaKeywords = $config['cf_title'].','.str_replace(' ', ',', $itemData['wr_subject']);    
} else if(!empty($bo_table)){
    $boardData = sql_fetch("SELECT * FROM g5_board WHERE bo_table = '$bo_table'");
...
<meta name="description" content="<?php echo $metaDescraiption?>" />
<meta property="og:image" content="<?php echo G5_URL?>/logo.jpg"/>
<meta property="og:url" content="<?php echo G5_URL?>"/>
<meta property="og:description" content="<?php echo $metaDescraiption?>"/>
