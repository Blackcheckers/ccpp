<?php
function latestNew($skin_dir='', $bo_table, $rows=10, $subject_len=40, $cache_time=1, $options='')
{
    global $g5;
    ...
    if( $caches === false ){
        ...
        $categoryWhere = " AND TRUE";
        if($category){
            $categoryWhere .= " AND ca_name = '$category'";
        }
        $sql = " select * from {$tmp_write_table} where wr_is_comment = 0 $categoryWhere order by wr_num limit 0, {$rows} ";
